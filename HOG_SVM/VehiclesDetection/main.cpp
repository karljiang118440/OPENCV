  ...
  //若TRAIN为true，重新训练分类器
	if(TRAIN)
	{
		...
		for(int num=0; num<PosSamNO && getline(finPos,ImgName); num++)
		{
			...
			ImgName = "/media/jcq/study/opencv/Projects/DATA/vehicles/KITTI_extracted/" + ImgName;//加上正样本的路径名
...
//Version_0			
//ImgName = "dataset/neg/" + ImgName;//加上负样本的路径名			Mat src = imread(ImgName);//读取图片

//Version_1				
ImgName = "/media/jcq/study/opencv/Projects/DATA/non-vehicles/Extras/" + ImgName;//加上负样本的路径名			
        ...
        ImgName = "/media/jcq/study/opencv/Projects/DATA/non-vehicles/GTI/" + ImgName;//加上HardExample负样本的路径名
				Mat src = imread(ImgName);//读取图片
				//resize(src,src,Size(64,128));
	...
	/*	ofstream fout("SampleFeatureMat.txt");
		...
		cout<<"训练完成"<<endl;
		svm.save("VehicleDetection.xml");//将训练好的SVM模型保存为xml文件

	}
	else //若TRAIN为false，从XML文件读取训练好的分类器
	{
		svm.load("VehicleDetection.xml");//从XML文件读取训练好的SVM模型
	}
